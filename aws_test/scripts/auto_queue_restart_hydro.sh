#!/bin/bash
# Auto Queue & Restart System - HYDROTHERMAL
# ===========================================
# 
# Based on successful miller_urey auto_queue_restart.sh
# Manages hydrothermal simulations with automatic restart
# 
# Features:
...
RESULTS_BASE="results/phase2b_additional/hydrothermal_extended"
CONFIG="aws_test/configs/phase2_hydrothermal_AWS_OPTIMIZED.yaml"
MAX_PARALLEL=4
CHECK_INTERVAL=300  # 5 minutes

# Run configuration
TOTAL_RUNS=17  # Minimum statistical sample (can increase to 30 later)
RUNS=($(seq 1 $TOTAL_RUNS))
SEEDS=(100 101 102 103 104 105 106 107 108 109 110 111 112 113 114 115 116)  # run_1=100, run_2=101, etc.

# Log files
MAIN_LOG="$BASE_DIR/logs/auto_restart_hydro_main.log"
PROGRESS_LOG="$BASE_DIR/logs/auto_restart_hydro_progress.log"
...
get_running_count() {
    ps aux | grep "run_phase2_full.py" | grep "hydrothermal" | grep -v grep | wc -l
}
...
is_run_in_progress() {
    local run_num=$1
    ps aux | grep "run_phase2_full.py" | grep "hydrothermal" | grep "run_$run_num" | grep -v grep > /dev/null
    return $?
}
...
start_run() {
    ...
    local seed=$2
    local run_dir="$BASE_DIR/$RESULTS_BASE/run_$run_num"
    local log_file="$BASE_DIR/logs/hydro_run_${run_num}.log"
    ...
    echo "$pid" > "$BASE_DIR/logs/hydro_run_${run_num}.pid"
}
...
check_stuck_processes() {
    ...
    for pid_file in "$BASE_DIR"/logs/hydro_run_*.pid; do
        if [ -f "$pid_file" ]; then
            pid=$(cat "$pid_file")
            run_num=$(basename "$pid_file" | sed 's/hydro_run_//' | sed 's/.pid//')
            log_file="$BASE_DIR/logs/hydro_run_${run_num}.log"
...
log "=========================================="
log "HYDROTHERMAL AUTO-RESTART SYSTEM"
log "=========================================="
log "Configuration:"
