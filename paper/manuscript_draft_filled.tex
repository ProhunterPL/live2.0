...
\begin{abstract}
% 250 words max
% Structure: Background (2-3 sentences) â†’ Methods (3-4 sentences) â†’ 
%            Results (4-5 sentences) â†’ Significance (2-3 sentences)
...
We use periodic boundary conditions with a simulation box of $100 \times 100$ Ă… for systems with $\sim$300-650 atoms, corresponding to realistic molecular densities.
...
\end{equation}
with $\alpha = 0.01$ Ă…$\cdot$fs$^2$/amu ensuring numerical stability.
...
Across all 30 simulations (10 replicates Ă— 3 scenarios), we detected a total of [XX] unique molecular species, ranging from simple diatomics to complex organics with up to [YY] heavy atoms. Molecular diversity increased nonlinearly over time, with the steepest accumulation occurring during the first 200,000 steps (Figure 3A).

Miller-Urey conditions produced [XX Â± YY] species per run (mean Â± SD across 10 replicates), significantly different from hydrothermal ([AA Â± BB], Kruskal-Wallis p = [P]) and formamide ([CC Â± DD], p = [P]) environments. The formamide scenario exhibited the highest molecular diversity, consistent with its richer starting composition (Figure 3A).
...
To quantify chemical diversity, we computed Shannon entropy H = -ÎŁp_i log(p_i) where p_i is the relative abundance of species i. Entropy increased logarithmically in all scenarios, reaching H = [X.X Â± Y.Y] (Miller-Urey), [A.A Â± B.B] (Hydrothermal), and [C.C Â± D.D] (Formamide) at 500,000 steps (Figure 3C). The higher entropy in formamide suggests more evenly distributed molecular populations.
...
Degree distributions followed power-law-like behavior (Figure 4D), suggesting scale-free network properties. The exponent Îł varied by scenario: Miller-Urey (Îł = [X.X Â± Y.Y]), Hydrothermal (Îł = [A.A Â± B.B]), Formamide (Îł = [C.C Â± D.D]). Formamide networks exhibited the longest tails, indicating more "super-hub" molecules.

Quantitative network metrics confirmed scenario differences (Table S2). Formamide networks had highest average degree ([X.X Â± Y.Y] vs [A.A Â± B.B] Miller-Urey, p < 0.001) and shortest path lengths ([L_f] vs [L_m], p < 0.01), suggesting denser, more interconnected chemistry.
...
We systematically searched for autocatalytic cycles using modified Johnson's algorithm on reaction networks. A cycle was classified as autocatalytic if it produced more copies of at least one reactant than were consumed. Across 30 simulations, we detected 769,315 unique autocatalytic cycles, ranging from direct autocatalysis (A + B â†’ 2A) to complex multi-step networks (Figure 5A).

Autocatalytic cycle frequency differed significantly: Miller-Urey ([X Â± Y] cycles/run), Hydrothermal ([A Â± B], p = [P]), Formamide ([C Â± D], p = [P], Figure 5B). Formamide exhibited the highest cycle frequency, with some replicates containing >10 distinct autocatalytic pathways.

Cycles were classified by topology: simple loops (2-3 nodes, [XX]\% of total), medium loops (4-6 nodes, [YY]\%), and complex networks (>6 nodes, [ZZ]\%). Direct autocatalysis (A + B â†’ 2A + C) was rare (1,199 instances), while indirect cycles involving intermediates were common (Figure 5C shows representative examples).

We quantified autocatalytic amplification by tracking molecule copy numbers over time. Amplification factors (final/initial abundance) ranged from 1.11 to 6, with median 1.43 (IQR: 1.32-1.58). The strongest amplifiers were [molecule names] in [scenario], reaching 6-fold amplification (Figure 5D).

Several detected cycles resembled the formose reaction (formaldehyde autocatalysis). In [N] formamide runs, we observed glycolaldehyde-mediated autocatalysis: CH$_2$O + CH$_2$O(OH)CHO â†’ sugars + more CH$_2$O(OH)CHO, validating our benchmark tests (Section 2.4.1). Maximum glycolaldehyde amplification: [XX]-fold over [time] steps.
...
Novel molecules comprised [YY]\% of total species, with median mass [M] amu (range: 1.11-6). They appeared later in simulations: median first detection at [XXX,XXX] steps vs [YYY,YYY] for known molecules (p < 0.001, Mann-Whitney), suggesting they arise from multi-step synthesis (Figure 6A).
